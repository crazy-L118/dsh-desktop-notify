...
/**
 * Collapse each line's whitespace while preserving the newlines between
 * structured lines (workspace/session labels, reply preview). Long inputs get
 * truncated at a line boundary when possible so the heading stays intact.
 */
function condense(text) {
	...
	let cut = joined.slice(0, MAX_BODY_CHARS - 1);
	const lastBreak = cut.lastIndexOf("\n");
	if (lastBreak > MAX_BODY_CHARS / 2) cut = cut.slice(0, lastBreak);
	return `${cut.trimEnd()}…`;
}
 ...
 */
async function sendWindowsNotification(title, body, sound, icon) {
  ...
  const audioTag = sound
    ...
    : '<audio silent="true"/>';
  const imageTag = icon
    ? `<image src="${powershellEscape(icon)}" placement="appLogoOverride"/>`
    : "";
  const script = `
...
$b = "${b}"
$a = "${a}"
$imageTag = "${powershellEscape(imageTag)}"
$sent = $false
try {
  [void][Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime]
  [void][Windows.Data.Xml.Dom.XmlDocument, Windows.Data.Xml.Dom.XmlDocument, ContentType = WindowsRuntime]
  $audio = '${audioTag}'
  $xmlText = '<toast><visual><binding template="ToastGeneric">' + $imageTag + '<text>' + [System.Security.SecurityElement]::Escape($t) + '</text><text>' + [System.Security.SecurityElement]::Escape($b) + '</text></binding></visual>' + $audio + '</toast>'
...
/** macOS strategy: AppleScript user notification. Custom icons are not supported. */
async function sendMacNotification(title, body, sound, icon) {
  // `icon` is intentionally ignored: macOS `display notification` does not support a custom icon.
  void icon;
...
async function sendLinuxNotification(title, body, icon) {
  const args = ["-a", "dsh"];
  if (icon) args.push("-i", String(icon));
  args.push(title, body);
  return runOnce("notify-send", args);
}
 ...
 * @param {string} [payload.icon] - Optional icon URL/path for the toast (OS-dependent support).
...
export async function sendDesktopNotification(payload) {
  ...
  try {
    if (process.platform === "win32") {
      const result = await sendWindowsNotification(title, body || "(no text reply)", sound, icon);
      return { ...result, strategy: "windows-toast" };
    }
    if (process.platform === "darwin") {
      const result = await sendMacNotification(title, body || "(no text reply)", sound, icon);
      return { ...result, strategy: "macos-notification" };
    }
    const result = await sendLinuxNotification(title, body || "(no text reply)", icon);
